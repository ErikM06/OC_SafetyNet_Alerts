...
public interface PersonRepository extends CrudRepository<Person, Integer> {
	...
	@Query(value = "SELECT new com.safetyNet.safetyNetAlerts.DTO.PersonInfoDTO ( m as medicalRecord, p.address as address,"
			+ " p.email as email) "
	...
	@Query(value = "SELECT new com.safetyNet.safetyNetAlerts.DTO.PersonInfoDTO ( m as medicalRecord, p.address as address,"
			+ " p.email as email) FROM MedicalRecord m INNER JOIN Person p ON m.firstName = p.firstName AND m.lastName = p.lastName"
			+ " WHERE  p.lastName =?1 ")
